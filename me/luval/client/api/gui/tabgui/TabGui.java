package me.luval.client.api.gui.tabgui;
...
import me.luval.client.api.event.Priority;
...
import me.luval.client.impl.modules.HUD;
		...
		if(!HUD.tabGui.getBooleanValue())
		 ...
		 this.renderTopString(5, 5);
	        ...
	        for (Category c : this.categoryValues) {
	            ...
	            fr.drawString(name.substring(0, 1).toUpperCase() + name.substring(1, name.length()).toLowerCase(),
	        ...
	        if (screen == 1 || screen == 2) {
	            ...
	            for (Module m : getModsForCurrentCategory()) {
	                ...
	                fr.drawString(m.getName() + (Luval.INSTANCE.SETTING_MANAGER.getSettingsForModule(m) != null ? ">" : ""), startModsX + 2 + (this.getCurrentModule().equals(m) ? 2 : 0),
